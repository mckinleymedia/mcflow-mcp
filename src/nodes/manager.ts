 ...
 * - JSON configurations → workflows/nodes/json/
...
type NodeType = 'code' | 'prompt' | 'sql' | 'template' | 'json';
...
`;
  ...
  /**
   * Extract JSON configuration from HTTP Request nodes
   */
  private async extractJSONNode(node: any, workflowName: string): Promise<ExtractedNode | null> {
    // Check if this is an HTTP Request node with JSON body
    if (node.type !== 'n8n-nodes-base.httpRequest') {
      return null;
    }

    const jsonBody = node.parameters?.jsonBody || node.parameters?.body;

    // Skip if no JSON body or if it's already extracted
    if (!jsonBody || typeof jsonBody !== 'string' || jsonBody === '' || node.parameters._nodeFile) {
      return null;
    }

    // Skip if it's just a simple reference or expression
    if (jsonBody.length < 50 || !jsonBody.includes('{')) {
      return null;
    }

    const safeNodeName = this.sanitizeFilename(node.name);
    const fileName = `${safeNodeName}.json`;
    const folderPath = path.join(this.nodesBasePath, 'json', workflowName);
    ...
    // Extract the JSON content (remove n8n expression prefix if present)
    let jsonContent = jsonBody;
    ...
    try {
      ...
      // Parse and reformat (this will fail if there are n8n expressions, which is fine)
      const parsed = JSON.parse(jsonContent);
      jsonContent = JSON.stringify(parsed, null, 2);
    } catch {
      // If parsing fails (likely due to n8n expressions), keep original format
      // but try to make it more readable
      jsonContent = jsonContent.replace(/\\n/g, '\n').replace(/\\t/g, '  ');
    }

    // Add header comment
    const header = `// JSON Configuration for: ${node.name}
// Workflow: ${workflowName}
// Node Type: ${node.type}
// Generated by McFlow
//${'-'.repeat(50)}
...
`;
    ...
    await fs.writeFile(filePath, header + jsonContent);

    // Clear JSON content and store file reference
    const relativeFilePath = path.relative(this.workflowsPath, filePath);
    if (node.parameters.jsonBody !== undefined) {
      node.parameters.jsonBody = '';
    } else if (node.parameters.body !== undefined) {
    ...
    return {
      ...
      subType: 'config',
      filePath: relativeFilePath,
      hash: this.hashContent(jsonContent)
    };
  }
        ...
        try {
          ...
          switch (node.type) {
            ...
            case 'n8n-nodes-base.httpRequest':
              ...
              if (nodeFilePath.endsWith('.json')) {
                // Format JSON for n8n (with = prefix and proper escaping)
                const jsonFormatted = '=' + fileContent.replace(/\n/g, '\\n').replace(/"/g, '\\"');
                node.parameters.jsonBody = jsonFormatted;
              }
              break;
          }
  ...
  private formatExtractionResults(results: any[]): string {
    ...
    let totals: Record<NodeType, number> = { code: 0, prompt: 0, sql: 0, template: 0, json: 0 };
